...
vi.mock('@/lib/api-middleware', async () => {
  ...
  return {
    withAuth: (handler: (req: NextRequest, ctx: { params: Promise<Record<string, string>>; user: { id: number; email: string; role: string } }) => Promise<NextResponse> | NextResponse) => async (req: NextRequest, ctx: { params: Promise<Record<string, string>> }) => {
      try {
        const user = { id: 1, email: 'test@test.com', role: 'customer' as const };
...
const { POST, PUT, DELETE } = await import('@/app/api/v1/cart/route');
...
describe('Cart API Route — ServiceError 回归测试', () => {
  ...
  describe('POST /api/cart', () => {
    it('产品不存在 → 404', async () => {
      ...
      const res = await POST(makeRequest('/api/cart', 'POST', { productId: 999, quantity: 1 }), { params: Promise.resolve({}) });
    ...
    it('产品已下架 → 422', async () => {
      ...
      const res = await POST(makeRequest('/api/cart', 'POST', { productId: 1, quantity: 1 }), { params: Promise.resolve({}) });
    ...
    it('库存不足 → 422', async () => {
      ...
      const res = await POST(makeRequest('/api/cart', 'POST', { productId: 1, quantity: 100 }), { params: Promise.resolve({}) });
      expect(res.status).toBe(422);
    });

    it('正常添加 → 201', async () => {
      mockCartService.addItem.mockResolvedValue({ id: 1, quantity: 2 });
      const res = await POST(makeRequest('/api/cart', 'POST', { productId: 1, quantity: 2 }), { params: Promise.resolve({}) });
  ...
  describe('PUT /api/cart', () => {
    it('购物车项不存在 → 404', async () => {
      ...
      const res = await PUT(makeRequest('/api/cart', 'PUT', { id: 999, quantity: 5 }), { params: Promise.resolve({}) });
      expect(res.status).toBe(404);
    });

    it('数量超限 → 422', async () => {
      mockCartService.updateQuantity.mockRejectedValue(new BusinessRuleError('数量必须在 1-99 之间'));
      const res = await PUT(makeRequest('/api/cart', 'PUT', { id: 1, quantity: 100 }), { params: Promise.resolve({}) });
      expect(res.status).toBe(422);
    });

    it('正常更新 → 200', async () => {
      mockCartService.updateQuantity.mockResolvedValue({ id: 1, quantity: 5 });
      const res = await PUT(makeRequest('/api/cart', 'PUT', { id: 1, quantity: 5 }), { params: Promise.resolve({}) });
  ...
  describe('DELETE /api/cart', () => {
    it('购物车项不存在 → 404', async () => {
      ...
      const res = await DELETE(makeRequest('/api/cart?id=999', 'DELETE'), { params: Promise.resolve({}) });
      expect(res.status).toBe(404);
    });

    it('正常删除 → 200', async () => {
      mockCartService.removeItem.mockResolvedValue(true);
      const res = await DELETE(makeRequest('/api/cart?id=1', 'DELETE'), { params: Promise.resolve({}) });
      expect(res.status).toBe(200);
    });
  });
