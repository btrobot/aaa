...
vi.mock('@/lib/api-middleware', async () => {
  ...
  return {
    withAdmin: (handler: Function) => async (req: NextRequest) => {
      try { return await handler(req); } catch (error) { return getErrorResponse(error); }
    },
    withMiddleware: (handler: Function) => async (req: NextRequest) => {
      try { return await handler(req); } catch (error) { return getErrorResponse(error); }
    },
...
describe('Settings API Route — ServiceError 回归测试', () => {
  ...
  describe('PUT /api/settings', () => {
    it('正常更新设置 → 200', async () => {
      ...
      const res = await PUT(makeRequest('/api/settings', 'PUT', { settings: { store_name: 'NewStore' } }));
      expect(res.status).toBe(200);
    });

    it('设置不存在 → 404', async () => {
      mockSettingsService.updateAll.mockRejectedValue(new NotFoundError('设置', 'missing_key'));
      const res = await PUT(makeRequest('/api/settings', 'PUT', { settings: { missing_key: 'val' } }));
      expect(res.status).toBe(404);
    });

    it('参数格式错误 → 400', async () => {
      const res = await PUT(makeRequest('/api/settings', 'PUT', { settings: 'not_an_object' }));
      expect(res.status).toBe(400);
    });

    it('设置数据为空 → 422', async () => {
      mockSettingsService.updateAll.mockRejectedValue(new BusinessRuleError('设置数据不能为空'));
      const res = await PUT(makeRequest('/api/settings', 'PUT', { settings: {} }));
      expect(res.status).toBe(422);
    });

    it('未捕获错误 → 500', async () => {
      mockSettingsService.updateAll.mockRejectedValue(new Error('未知错误'));
      const res = await PUT(makeRequest('/api/settings', 'PUT', { settings: { key: 'val' } }));
