            ...
            {wishlist.map((item) => (
              <Card key={item.id} className="border-0 shadow-sm group">
                ...
                <CardContent className="p-3">
                  <Link href={`/${locale}/products/${item.productId}`}>
                    <h3 className="text-sm font-medium text-gray-900 line-clamp-2 hover:text-blue-600">{t('common.product')} #{item.productId}</h3>
                  </Link>
                  <Button size="sm" className="w-full mt-2 bg-blue-600 hover:bg-blue-700 text-xs" asChild>
                    <Link href={`/${locale}/cart?add=${item.productId}`}>
